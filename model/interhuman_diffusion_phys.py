...
class interhuman_diffusion_phys(interhuman_diffusion):
    ...
    def proxemics_infer(self, data, step=0, eta=0.0):
        batch_size, frame_length, agent_num = data['features'].shape[:3]
        num_valid = batch_size * frame_length * agent_num
        ...
        init_pose = data['init_pose_6d']
        _, mean = self.generate_noise(init_pose)
        ...
        # construct noise from current optimization variables
        pred_pose6d = data['curr_pose_6d'].reshape(-1, 144)
        pred_shape = data['curr_betas'].reshape(-1, 10)
        full_cam = data['curr_transl'].reshape(-1, 3)
        pred_cam = cam_full2crop(full_cam, center, scale, full_img_shape, focal_length)
        ...
        viz_denoising = False
        if viz_denoising and step % 100 == 0:
            step_name = '%05d' %step
            self.visualize(pred_pose6d, pred_shape, pred_cam, data, img_info, 0, name=step_name)

        x_1 = torch.cat([pred_pose6d, pred_shape, pred_cam], dim=-1)
        x_1 = x_1.reshape(*mean.shape)
        x_1 = x_1 - mean

        t = torch.tensor([4] * x_1.shape[0], device=x_1.device)
        new_ts = torch.tensor([80] * x_1.shape[0], device=x_1.device)
        ...
        model_variance = extract_into_tensor(model_variance, t, x_1.shape)
        model_log_variance = extract_into_tensor(model_log_variance, t, x_1.shape)

        pred_xstart = x_1

        model_mean, _, _ = self.q_posterior_mean_variance(
            x_start=pred_xstart, x_t=x_1, t=t
        )

        assert (
            model_mean.shape == model_log_variance.shape == pred_xstart.shape == x_1.shape
        )

        # Usually our model outputs epsilon, but we re-derive it
        # in case we used x_start or x_prev prediction.
        eps = self._predict_eps_from_xstart(x_1, t, pred_xstart)

        alpha_bar = extract_into_tensor(self.test_alphas_cumprod, t, x_1.shape)
        alpha_bar_prev = extract_into_tensor(self.test_alphas_cumprod_prev, t, x_1.shape)
        sigma = (
                eta
        ...
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x_1.shape) - 1)))
        )  # no noise when t == 0
        ...
        if viz_sampling:
            ...
            self.visualize(pose, shape, pred_cam, data, img_info, 4, name='x1')

        pred = self.inference(x_1, new_ts, cond, img_info, data, mean)

        # Visualize each diffusion step
        viz_denoising = False
        if viz_denoising and step % 1 == 0:
            step_name = '%05d_updated' %step
            pred_pose6d = pred['pred_pose6d']
            pred_shape = pred['pred_shape']
            pred_cam = pred['pred_cam']
            self.visualize(pred_pose6d, pred_shape, pred_cam, data, img_info, 0, step_name)

        updated_pose = pred['pred_pose'].reshape(batch_size, frame_length, agent_num, -1)
        updated_transl = pred['pred_cam_t'].reshape(batch_size, frame_length, agent_num, -1)
        updated_betas = pred['pred_shape'].reshape(batch_size, frame_length, agent_num, -1)

        updated_pose = torch.cat([updated_pose[:,:,0], updated_pose[:,:,1]], dim=1).reshape(-1, 72)
        updated_transl = torch.cat([updated_transl[:,:,0], updated_transl[:,:,1]], dim=1).reshape(-1, 3)
        updated_betas = torch.cat([updated_betas[:,:,0], updated_betas[:,:,1]], dim=1).reshape(-1, 10)

        return updated_pose, updated_transl, updated_betas
