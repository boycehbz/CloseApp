...
def to_cuda(items: dict, device, add_batch = False, precision = torch.float32):
    ...
    for key, data in items.items():
        if isinstance(data, torch.Tensor):
            items_cuda[key] = data[0].to(device)
...
def load_masks(PROJECT_DIR, posmap_size, body_model='smpl'):
    uv_mask_faceid = np.load(join(PROJECT_DIR, 'data', 'uv_masks', 'uv_mask{}_with_faceid_{}.npy'.format(posmap_size, body_model))).reshape(posmap_size, posmap_size)
    uv_mask_faceid = torch.from_numpy(uv_mask_faceid).long().cuda()
    
    smpl_faces = np.load(join(PROJECT_DIR, 'data', '{}_faces.npy'.format(body_model.lower()))) # faces = triangle list of the body mesh
...
def load_barycentric_coords(PROJECT_DIR, posmap_size, body_model='smpl'):
    ...
    '''
    bary = np.load(join(PROJECT_DIR, 'data', 'bary_coords_uv_map', 'bary_coords_{}_uv{}.npy'.format(body_model, posmap_size)))
